...
export function WasteSelectionModal({weekday, weekNumber, fn}: Props) {
  return (
    <div className="fixed z-50 top-0 left-0 h-[100dvh] w-[100vw] bg-black bg-opacity-50 backdrop-blur-md">
      <div className="flex flex-col justify-center gap-10 h-full w-full md:w-[80vw] mx-auto px-10">
        <p className=" text-center">Seleziona i tipi di rifiuti da conferire {weekday.toLowerCase()}{weekNumber && ` della settimana ${weekNumber}`}</p>
        <div className="grid grid-flow-row grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-x-20 gap-y-10 justify-center justify-items-center">
          {wasteTypes.map((wasteType) => (
            <WasteType wasteType={wasteType.type} bgColor={`${wasteType.bgColor}`} icon={wasteType.icon} forWeekday={weekday} forWeekNumber={weekNumber} />
          ))}
        </div>
